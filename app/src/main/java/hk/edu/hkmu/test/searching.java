...
public class searching extends AppCompatActivity {
    ...
    static void ensearchname(String searchstr){
        ...
        for(int i =0; i< SchoolInfo.eninfoList.size();i++){
            if(SchoolInfo.eninfoList.get(i).get("English Name").toLowerCase().contains(searchstr.toLowerCase())){  //check if text match any school name
    ...
    static void chsearchname(String searchstr){
        ...
        for(int i =0; i< SchoolInfo.chinfoList.size();i++){
            if(SchoolInfo.chinfoList.get(i).get("名稱").contains(searchstr)){  //check if text match any school name
                HashMap<String, String> search = new HashMap<>();
